...
factories = [factory_1, factory_2, factory_3, factory_4, factory_5, factory_6, factory_7, factory_8, factory_9,
...
def overhead_d_func(scheme, num_logical_qubits, T_count):
    ...
    overhead_d = {"space(d^2)": tiles, "time(d)": timesteps, "spacetime(d^3)": tiles * timesteps}
...
def overhead_phys_func(overhead_d, p_phys, t_cycle):
    ...
    overhead_phys = {"space(physical qubits)": qubits, "time(sec)": time, "spacetime(qubitsecs)": qubits * time,
                     "distance_used": d}
...
def overheads_func(scheme, num_logical_qubits, T_count, p_phys, t_cycle):
    ...
    overheads = {
        "physical qubits": int(overhead_phys['space(physical qubits)'] + best_factory['Qubits'] * num_facs),
...
parser.add_argument("-c", "--config", required=True,
                    help="Specifies the configuration file (by default, reads from config.json)")

# Read arguments from command line
args = parser.parse_args()

if __name__ == '__main__':
    n_qubits = int(sys.stdin.readline())
    tmp = sys.stdin.readline()
    t_counts = [int(x) for x in tmp[:-1].lstrip('[').rstrip(']').split(',')]
